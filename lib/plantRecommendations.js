 ...
 */
export function getRecommendations(gardenPlantIds = [], customPlants = []) {
  ...
  // Collect existing names from both catalog plants and custom plants to avoid duplicates
  const existingNames = new Set([
    ...PLANTS.filter(p => gardenPlantIds.includes(p.id)).map(p => p.name.toLowerCase()),
    ...customPlants.map(n => n.toLowerCase()),
  ])
